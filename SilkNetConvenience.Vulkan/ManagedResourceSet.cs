...
public unsafe class ManagedResources : IDisposable {
	...
	public TOut* AllocateCreateInfos<TOut>(IEnumerable<IGetCreateInfo<TOut>> createInfos) 
		where TOut : unmanaged {
		var data = createInfos.ToArray();
		if (data.Length == 0) return null;
		
		var ptr = Marshal.AllocHGlobal(Marshal.SizeOf<TOut>() * data.Length);
		_hGlobals.Add(ptr);

		var span = new Span<TOut>((void*)ptr, data.Length);
		var managedResource = data.Select(c => c.GetCreateInfo()).ToArray();
	...
	public T* AllocateStruct<T>(T? structure) where T : unmanaged {
		if (!structure.HasValue) return null;
		
		var ptr = Marshal.AllocHGlobal(sizeof(T));
		_hGlobals.Add(ptr);
		var span = new Span<T>((void*)ptr, 1);
		new[] { structure.Value }.AsSpan().CopyTo(span);
		return (T*)ptr;
	}
}